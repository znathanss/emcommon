...
class Item:
    ...
    def info(self, field):
        ...
        if not self.redis_conn:
            request_url = f"https://esi.evetech.net/latest/\
            universe/types/{self.type_id}/?datasource=tranquility&language=en"
    ...
    def orders(self, system_id=None):
        ...
        if system_id is None:
            request_url = f"https://api.eve-market.net/\
            orders?item_id={self.type_id}"
        else:
            request_url = f"https://api.eve-market.net/\
            orders?item_id={self.type_id}&system_id={system_id}"
    ...
    def price(self, system_id=30000142):
        ...
        request_url = f"https://api.eve-market.net/\
        get_adjusted_market_price?type_id={self.type_id}&market={system_id}"
        data = json.loads(requests.get(request_url).text)
        return data
